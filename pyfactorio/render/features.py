...
SCREEN_FEATURES = ScreenFeatures(
    ...
    unit_type=(max(static_data.UNIT_TYPES) + 1, FeatureType.CATEGORICAL, colors.unit_type, False),
...
"""
    waiting_for_source_item=(2, FeatureType.CATEGORICAL, colors.hot, False),
    waiting_for_space_in_dest=(2, FeatureType.CATEGORICAL, colors.hot, False),
"""
)
...
def parse_agent_interface_format(
  ...
  Args:
    ...
    feature_map: If specified, so must feature_screen be.
...
class Features(object):
  ...
  def transform_obs(self, obs):
    ...
    if aif.feature_dimensions:
      ...
      with sw("feature_map"):
        out["feature_map"] = named_array.NamedNumpyArray(
            np.stack([or_zeros(f.unpack(obs.observation),
                               aif.feature_dimensions.minimap)
                      for f in MINIMAP_FEATURES]),
            names=[MapFeatures, None, None])
