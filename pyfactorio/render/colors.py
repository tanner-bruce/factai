...
TREE_PALETTE = numpy.array([black, green])
WATER_PALETTE = numpy.array([black, blue])
ROCK_PALETTE = numpy.array([black, red * 0.6])
SELECTED_PALETTE = numpy.array([black, green * 0.7])

def item_type(scale=None):
  """Returns a palette that maps unit types to rgb colors."""
  return categorical(static_data.ITEM_TYPES, scale)
